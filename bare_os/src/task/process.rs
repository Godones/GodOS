...
impl ProcessControlBlock {
    pub fn new(data: &[u8]) -> Arc<Self> {
        ...
        let main_task = Arc::new(TaskControlBlock::new(
            process.clone(), ustack_base,true));
        //创建主线程的trap上下文
    ...
    pub fn spawn(self: &Arc<ProcessControlBlock>, path: &str) -> isize {
        ...
        if data.len()!=0 {
            ...
            self.get_inner_access().children.push(process_control_block.clone());
            drop(inner);
    ...
    pub fn fork(self: &Arc<Self>) -> Arc<Self> {
        ...
        let main_task = Arc::new(TaskControlBlock::new(
            child.clone(),
            parent_inner.get_task(0).get_inner_access().res.as_ref().unwrap().ustack_base,
            false));//不需要重新申请用户栈和trap上下文
