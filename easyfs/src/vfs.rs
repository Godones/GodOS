...
impl Inode {
    pub fn clear(&self) {
        ...
        self.modify_disk_inode(|disk_node|{
            ...
            //判断回收回来的数据块是否正确
            assert_eq!(block_realloc_ids.len(), DiskNode::total_blocks(size) as usize);
            block_realloc_ids.into_iter().for_each(|indx|{
                fs.dealloc_data(indx)
            });//回收数据块，将会清0
        })
    }
    pub fn read_at(&self,offset: usize,buf: &mut [u8])->usize{
        ...
        self.read_disk_inode(|disk_node|{
            disk_node.read_at(offset,buf,&self.block_device)
        })
    }
